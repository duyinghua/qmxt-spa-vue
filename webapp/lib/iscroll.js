    ...
    // Constructor
        iScroll = function (el, options) {
            ...
            that.options = {
                ...
                handleClick: false,
                edgeRestorePrevent: false,//@dyh--old:empty

                // Events
                onRefresh: null,
                onBeforeScrollStart: null,//@dyh--old: function (e) { e.preventDefault(); },
                onScrollStart: null,
            ...
            that.scroller.setAttribute('x', that.x);//@dyh 保存值
            that.scroller.setAttribute('y', that.y);//@dyh 保存值
            that.refresh();
...
// Prototype
    iScroll.prototype = {
        ...
        isTop: false,    //@dyh--old:empty
        isBottom: false, //@dyh--old:empty
        isVerticalScrolling: false,//@dyh--old:empty 正在按住页面默认垂直滚动
        ...
        _pos: function (x, y) {
            ...
            this.scroller.setAttribute('x', x);//@dyh 保存值
            this.scroller.setAttribute('y', y);//@dyh 保存值
            this.x = x;
            this.y = y;
        ...
        _move: function (e) {
            ...
            //@dyh--start 到边界就直接执行默认，否则阻止默认执行自己
            if (((that.isTop && deltaY > 0) || (that.isBottom && deltaY < 0)) && that.options.edgeRestorePrevent) {
                return;
            } else if (that.options.vScroll) {//@dyh old else
                e.preventDefault();
            }
            if (that.isVerticalScrolling) {//正在默认垂直滚动的话，阻止水平滚动，直接返回
                return;
            }
            if (!that.options.vScroll && that.options.hScroll && "ontouchend" in document) {//如果是只有水平滚动，按斜率阻止垂直滚动或水平滚动
                var k = m.abs(deltaX) / m.abs(deltaY || 0.1);
                if (k >= 1) {//水平
                    e.preventDefault();
                } else {//垂直
                    that.isVerticalScrolling = true;
                    return;
                }
            }
            //@dyh--end
        ...
        _end: function (e) {
            ...
            that.isVerticalScrolling = false;
        ...
        refresh: function () {
            ...
            that.maxScrollY = that.wrapperH - that.scrollerH + that.minScrollY + that.options.bottomOffset;
            that.dirX = 0;
            that.dirY = 0;
        ...
        $.fn.iScroll = function (opts) {
            ...
            $.each(this, function (i, el) {
                ...
                if (method === 'this') {
                    ...
                    method === 'destroy' && record(el, null);   //@dyh--old:empty 当调用毁灭方法时，从缓存中除去el
                    // 当取的方法不存在时，抛出错误信息
